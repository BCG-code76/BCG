...
from my_utils.unet_2d_condition import UNet2DConditionModel
...
def initialize_unet(rank, return_lora_module_names=False):
    ...
    unet = UNet2DConditionModel.from_pretrained("stabilityai/sd-turbo", subfolder="unet", cache_dir=cache_dir, ignore_mismatched_sizes=True, low_cpu_mem_usage=False, device_map=None)
...
class CycleGAN_Turbo(torch.nn.Module):
    def __init__(self, pretrained_name=None, pretrained_path=None, ckpt_folder="checkpoints", lora_rank_unet=8, lora_rank_vae=4):
        ...
        cond_vae = AutoencoderKL.from_pretrained("stabilityai/sd-turbo", subfolder="vae")
        unet = UNet2DConditionModel.from_pretrained("stabilityai/sd-turbo", subfolder="unet", ignore_mismatched_sizes=True, low_cpu_mem_usage=False, device_map=None)
        cond_unet = UNet2DConditionModel.from_pretrained("stabilityai/sd-turbo", subfolder="unet")
        vae.encoder.forward = my_vae_encoder_fwd.__get__(vae.encoder, vae.encoder.__class__)
        cond_vae.encoder.forward = my_vae_encoder_fwd.__get__(cond_vae.encoder, cond_vae.encoder.__class__)
        vae.decoder.forward = my_vae_decoder_fwd.__get__(vae.decoder, vae.decoder.__class__)
        # add the skip connection convs
        ...
        vae.decoder.ignore_skip = False
        self.unet, self.vae, self.cond_vae ,self.cond_unet= unet, vae, cond_vae, cond_unet
    ...
    def load_ckpt_from_state_dict(self, sd):
        ...
        cond_lora_conf_encoder = LoraConfig(r=sd["rank_unet"], init_lora_weights="gaussian", target_modules=sd["l_target_modules_encoder"], lora_alpha=sd["rank_unet"])
        cond_lora_conf_others = LoraConfig(r=sd["rank_unet"], init_lora_weights="gaussian", target_modules=sd["l_modules_others"], lora_alpha=sd["rank_unet"])

        self.unet.add_adapter(lora_conf_encoder, adapter_name="default_encoder")
        self.unet.add_adapter(lora_conf_decoder, adapter_name="default_decoder")
        self.unet.add_adapter(lora_conf_others, adapter_name="default_others")

        self.cond_unet.add_adapter(cond_lora_conf_encoder, adapter_name="default_encoder")
        self.cond_unet.add_adapter(cond_lora_conf_others, adapter_name="default_others")
        ...
        for n, p in self.cond_unet.named_parameters():
            name_sd = n.replace(".default_encoder.weight", ".weight")
            if "lora" in n and "default_encoder" in n:
                p.data.copy_(sd["sd_encoder"][name_sd])
        for n, p in self.cond_unet.named_parameters():
            ...
            if "lora" in n and "default_others" in n:
                p.data.copy_(sd["sd_other"][name_sd])
        self.cond_unet.set_adapter(["default_encoder", "default_others"])

        vae_lora_config = LoraConfig(r=sd["rank_vae"], init_lora_weights="gaussian", target_modules=sd["vae_lora_target_modules"])

        cond_vae_lora_config = LoraConfig(r=sd["rank_vae"], init_lora_weights="gaussian", target_modules=sd["vae_lora_target_modules"])
        ...
        self.cond_vae.add_adapter(cond_vae_lora_config, adapter_name="encoder_lora")
        self.cond_vae_b2a = copy.deepcopy(self.cond_vae)
        self.cond_vae_enc = VAE_encode(self.cond_vae, self.cond_vae_b2a)
        self.cond_vae_enc.load_state_dict(sd["sd_vae_enc"])
    ...
    @staticmethod
    def forward_with_networks(x, direction, vae_enc, unet, vae_dec, sched, timesteps, text_emb, condition=None):
        B = x.shape[0]
        assert direction in ["a2b", "b2a"]
    ...
    def forward(self, x_t, direction=None, caption=None, caption_emb=None):
        ...
        return self.forward_with_networks(x_t, direction, self.vae_enc, self.unet, self.vae_dec, self.sched, self.timesteps, caption_enc, condition)
