...
def main(args):
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            logs["hist_loss"] = hist_loss.detach().item()
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 1:
                        ...
                        sd["sd_hist_encoder"] = accelerator.unwrap_model(hist_encoder).state_dict()
                        torch.save(sd, outf)
                        gc.collect()
                        torch.cuda.empty_cache()
                    ...
                    if global_step % args.validation_steps == 1:
                        ...
                        for idx, input_img_path in enumerate(tqdm(l_images_src_test)):
                            ...
                            with torch.no_grad():
                                ...
                                a = net_dino.preprocess_a(input_img).unsqueeze(0).cuda()
                                b = net_dino.preprocess_b(eval_fake_b_pil).unsqueeze(0).cuda()
                        ...
                        ssim_score_a2b = np.mean(l_ssim_scores_a2b)
                        ...
                        # # src_images = []
                        # # for img_path in l_images_src_test:
                        # #     src_img = T_val(Image.open(img_path).convert("RGB"))
                        ...
                        #         img_b = transforms.Normalize([0.5], [0.5])(img_b)
                        #         # src_img = src_images[idx % len(src_images)]
                        #         # src_img = rgb2yuv(src_img)
                        #         # condition_b = load_hist(src_img, min_val=args.min_val, max_val=args.max_val, bin_num=args.bin_num)
                        #         # condition_b = condition_b.unsqueeze(0).cuda()
                        #         # condition_b = condition_b.to(dtype=eval_unet.dtype) 
                        #         condition_b = hist_encoder(img_b)
                        ...
                        #         a = net_dino.preprocess_a(input_img).unsqueeze(0).cuda()
                        #         b = net_dino.preprocess_b(eval_fake_a_pil).unsqueeze(0).cuda()
                        #         dino_ssim = net_dino.calculate_global_ssim_loss(a, b).item()
                        #         l_dino_scores_b2a.append(dino_ssim)
                        # dino_score_b2a = np.mean(l_dino_scores_b2a)
                        # ssim_score_b2a = np.mean(l_ssim_scores_b2a)
                        # gen_features = get_folder_features(fid_output_dir, model=feat_model, num_workers=0, num=None,
                        #     shuffle=False, seed=0, batch_size=8, device=torch.device("cuda"),
                        #     mode="clean", custom_fn_resize=None, description="", verbose=True,
                        #     custom_image_tranform=None)
                        ...
                        # gen_feat_sampled = gen_feat_tensor[:min_samples]
                        # ref_feat_sampled = ref_feat_tensor[:min_samples]
                        # cmmd_score_b2a = cmmd(gen_feat_sampled, ref_feat_sampled).item()
                        # print(f"step={global_step}, fid(b2a)={score_fid_b2a}, ssim(b2a)={ssim_score_b2a:.3f}, dino(b2a)={dino_score_b2a:.3f}, cmmd(b2a)={cmmd_score_b2a:.3f}")
                        # logs["val/fid_a2b"], logs["val/fid_b2a"] = score_fid_a2b, score_fid_b2a
                        # logs["val/ssim_a2b"], logs["val/ssim_b2a"] = ssim_score_a2b, ssim_score_b2a
                        # logs["val/dino_struct_a2b"], logs["val/dino_struct_b2a"] = dino_score_a2b, dino_score_b2a
                        # logs["val/cmmd_a2b"], logs["val/cmmd_b2a"] = cmmd_score_a2b, cmmd_score_b2a
