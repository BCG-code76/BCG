...
class BaseModel(ABC):
    ...
    def __init__(self, opt):
        ...
        # 添加 InceptionV3 模型初始化
        self.inception = inception_v3(pretrained=True, transform_input=False).to(self.device)
        self.inception.eval()  # 设置为评估模式
        # 移除分类层，只保留特征提取部分
        self.inception.fc = torch.nn.Identity()
        # 冻结参数，不需要更新
        self.set_requires_grad(self.inception, requires_grad=False)
    ...
    def calculate_ssim(self, real_images, fake_images):
        """Calculate SSIM between real and fake images."""
        return self.ssim_metric(fake_images, real_images)
    ...
    def extract_fid_features(self, x):
        """提取用于FID计算的特征"""
        # 预处理图像（调整大小并标准化）
        x = F.interpolate(x, size=(299, 299), mode='bilinear', align_corners=False)
        x = (x + 1) / 2  # 将[-1, 1]范围转换为[0, 1]
        x = x * 255.0    # 转换为[0, 255]
        # InceptionV3的标准化
    ...
    def calculate_fid_from_features(self, real_feats, fake_feats):
        """从特征计算FID距离"""
        ...
        return torch.tensor(fid, device=self.device)